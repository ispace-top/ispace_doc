/**
 * @deprecated 此文件中的编辑器逻辑已被内联编辑器体系替代。
 * 新的编辑器初始化和切换逻辑位于 frontend/templates/includes/inline_editor.html
 * 新的编辑器管理逻辑位于 doc.html / pro_list.html 的 enterEditMode/enterCreateMode 等函数。
 * getOptimizedSheets() 的功能已内联至 inline_editor.html 的 _getOptimizedSheets()。
 * 此文件中的其他辅助函数如不再使用，可安全移除。
 */
...
function linebody(count, name) {
  ...
  body += "<td><img src='/static/isdoc-editor/add.gif' name='" + name + "' style='cursor: pointer' onclick='InsertLine(this)'>" +
      "<img src='/static/isdoc-editor/delete.gif'  name='" + name + "' style='cursor: pointer' onclick='DeleteLine(this)'></td>";
  body += "</tr>";
...
function Insertrow(obj) {
  ...
  var td = "<td><div contenteditable='true' style='float: left;width: 70%'>列名</div><img src='/static/isdoc-editor/delete.gif'  name='" + name + "' style='cursor: pointer' onclick='Deleterow(this)'></td>"
  for (var i = 0; i < $(table_id).find("tr").length; i++) {
      if (i == 0) {
...
function addtable(btn) {
  ...
  for (var i = 0; i <= col; i++) {
      if (i == 0) {
          ...
          body += "<td><div contenteditable='true' style='float: left;width: 70%'>列名</div><img src='/static/isdoc-editor/delete.gif' name='" + name + "' style='cursor: pointer' onclick='Deleterow(this)'></td>";
...
function addsrc(id, name) {
  for (var i = 0; i < $(id).find("tr").length; i++) {
      if (i == 0) {
          var td = "<td class='small'><img src='/static/isdoc-editor/add.gif' name='" + name + "' style='cursor: pointer' onclick='Insertrow(this)'></td>";
          $(id).find("tr").eq(i).append(td);
      } else {
          var td = "<td><img src='/static/isdoc-editor/add.gif' name='" + name + "' style='cursor: pointer' onclick='InsertLine(this)'>" +
              "<img src='/static/isdoc-editor/delete.gif' name='" + name + "' style='cursor: pointer' onclick='DeleteLine(this)'></td>";
          $(id).find("tr").eq(i).append(td);
      }
...
function autoCacheDoc(){
  setInterval(() => {
      ...
      window.localStorage.setItem('isdoc_doc_cache',editor_value)
  }, 10000);
};

// 查看本地文档缓存
$("#doc-cache-btn").click(function(){
    var editor_cache = window.localStorage.getItem('isdoc_doc_cache') // 获取文档缓存内容
    ...
    if(editor_cache_cnt > 5){ // 文档缓存去除空格换行后长度大于5
        ...
        layer.open({
            ...
            yes:function(index, layero){
                ...
                window.localStorage.removeItem('isdoc_doc_cache')
                layer.closeAll()
                autoCacheDoc();
            },
...
//修改预览div中a标签链接新窗口打开(Vditor使用vditor-preview类)
$('div.vditor-preview').on('click','a',function(e){
...
// 监听文档div点击
var docBodyEl = document.querySelector('.doc-body');
if (docBodyEl) {
  docBodyEl.addEventListener('click', function (e) {
    var screen_width = window.matchMedia('(max-width: 768px)');
    // 小屏下收起左侧文档大纲
    if(screen_width.matches){
        // console.log("点击了div")
...
element.on('tab(img-tab)', function(data){
  ...
  if(data.index == 1){
    ...
    //请求新的分组数据
    $.post("/files/image-groups/",{'types':3},function(r){
        if(r.status){
            group_btn_str = ''
            for(var i in r.data){
    ...
    //请求全部图片数据
    $.post("/files/images/",{'types':2,'group_id':0},function(r){
        if(r.status){
            //调用分页显示
            laypage.render({
