# coding:utf-8
"""
Django settings for iSpaceDoc project.
...
# 配置文件和数据文件目录
CONFIG_DIR = os.path.join(BASE_DIR, 'config', 'conf')
CONFIG = ConfigParser()
...
INSTALLED_APPS = [
    ...
    'haystack', # 全文搜索
    'backend.apps.admin', # 管理APP
    'backend.apps.doc', # 文档APP
    'backend.apps.api', # API APP
...
if LANGUAGE_FORCE:
    MIDDLEWARE = [
        'backend.apps.admin.middleware.setup_middleware.SetupCheckMiddleware',
        'django.middleware.security.SecurityMiddleware',
        'django.contrib.sessions.middleware.SessionMiddleware',
        'django.middleware.locale.LocaleMiddleware',
        'backend.apps.admin.middleware.language.ForceDefaultLanguageMiddleware', # 强制使用设置的语言包
        ...
        'backend.apps.admin.middleware.require_login_middleware.RequiredLoginMiddleware',
        'backend.apps.admin.middleware.AuditLogMiddleware',
    ]
else:
    MIDDLEWARE = [
        'backend.apps.admin.middleware.setup_middleware.SetupCheckMiddleware',
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'context_processors': [
                ...
                'backend.apps.admin.context_processors.sys_setting', # 自定义系统设置上下文渲染
                'backend.apps.doc.context_processors.sidebar_tree', # 全局侧边栏文档树
            ],
            'libraries': { # 自定义的模板标签
                'doc_filter' : 'backend.apps.doc.templatetags.doc_filter',
                'project_filter' : 'backend.apps.doc.templatetags.project_filter',
            },
        },
    },
]
...
if db_engine == 'sqlite':
    ...
    # 环境变量可覆盖配置文件中的数据库密码
    db_name = os.environ.get('DB_NAME', CONFIG['database']['name'])
    db_user = os.environ.get('DB_USER', CONFIG['database']['user'])
    db_password = os.environ.get('DB_PASSWORD', CONFIG.get('database', 'password', fallback=''))
...
# 允许上传的图片后缀
ALLOWED_IMG = CONFIG.get("image_upload","suffix_name",fallback="jpg,jpeg,gif,png,bmp,webp,tiff").split(",")
...
HAYSTACK_CONNECTIONS = {
    'default': {
        # 使用whoosh引擎
        'ENGINE': 'backend.apps.doc.search.whoosh_cn_backend.WhooshEngine',
        # 索引文件路径
...
# 自定义高亮
HAYSTACK_CUSTOM_HIGHLIGHTER = "backend.apps.doc.search.highlight.MyHighLighter"
